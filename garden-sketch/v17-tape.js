(()=>{
  ...
  function install(){
    ...
    function approximateDirection(anchor,view){
      try{
    ...
    function screenTarget(anchor,view,ret){
      ...
      const d=approximateDirection(anchor,view);
      lastDirection=d;
    ...
    function draw(pose){
      ...
      let anchorScreen=null,hitScreen=null;
      try{
        anchorScreen=projectScreen(anchor,view);
        hitScreen=projectScreen(currentHit,view);
      }catch(e){}

      // The base application already draws the live tape when both endpoints are visible.
      // The fallback layer is used only after the saved point leaves the camera view.
      if(anchorScreen&&hitScreen&&anchorScreen.visible&&hitScreen.visible){
        svg.innerHTML='';
        return;
      }

      const ret=reticleCenter();
      const target=screenTarget(anchor,view,ret);
