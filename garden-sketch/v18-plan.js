(()=>{
  const state={align:'longest',turns:0,dimensions:true,format:'A4',orientation:'landscape',scale:'auto'};
  ...
  function geometry(){
    ...
    const fit=drawW<=plot.w&&drawH<=plot.h;
    const ox=plot.x+(plot.w-drawW)/2-b.minX*mmPerM;
    const oy=plot.y+(plot.h-drawH)/2+b.maxY*mmPerM;
